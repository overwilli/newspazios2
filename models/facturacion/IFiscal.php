...
class class_IFiscal {
    ...
    public function method_reporte_auditoria($params, $error) {
        global $conexion;
        ...
        $conexion = fsockopen($p->conexion->host, $p->conexion->port);
        if ($conexion) {
            $resultado[] = DailyCloseByDate($p->auditoria->fecha_desde, $p->auditoria->fecha_hasta, $p->auditoria->tipo);
            fclose($conexion);
        } else {
            return "no conecta";
        }
    ...
    public function method_cierreZ_numero($params, $error) {
        ...
        $conexion = fsockopen($p->conexion->host, $p->conexion->port);
        if ($conexion) {
            $resultado[] = DailyCloseByNumber($p->numero_inicial, $p->numero_final, $p->tipo_datos);
    ...
    public function method_documento_fiscal($params, $error) {
        ...
        require("ConvertCharset_v1.1/ConvertCharset.class.full.php");
        require("rutinas_ifiscal.php");
        ...
        if ($conexion) {
            ...
            if (!is_null($p->encabezado)) {
                foreach ($p->encabezado as $key => $value) {
                    $resultado[] = SetHeaderTrailer($key, $ConvertCharset->Convert($value));
                }
            }
            ...
            if (!is_null($p->cola)) {
                foreach ($p->cola as $key => $value) {
                    $resultado[] = SetHeaderTrailer($key + 10, $ConvertCharset->Convert($value));
                }
            }



            if (empty($p->cliente->nrodoc))
                $p->cliente->nrodoc = "1";
    ...
    public function method_documento_no_fiscal_homologado($params, $error) {
        ...
        $conexion = fsockopen($p->conexion->host, $p->conexion->port);
        if ($conexion) {
            if (!is_null($p->copias)) { echo "ingreso por null";exit;
                $resultado[] = ConfigureControllerByOne("9", $p->copias);
            }
