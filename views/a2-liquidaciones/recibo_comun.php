...
$concepto = " pago por per&iacute;odo: " . $model_liq->liq_mes . "/" . $model_liq->liq_anio . " C&oacute;digo: " . 
        $model_operacion->cod_propiedad;
$fecha_pago = $model_liq->fecha_pago;
if ($model_liq->operacionInmobiliaria) {    
    $nombre = $model_liq->operacionInmobiliaria->cliente->NOMBRE;
}
...
<table width="800" border="1" cellpadding="0" cellspacing="0" bordercolor="#333333">
    <tr><td>
            <table width="100%" border="0" cellpadding="2" cellspacing="2">
                ...
                if ($model_liq->liq_mes == 12) {
                    $month = 1;
                    $year = $model_liq->liq_anio + 1;
                } else {
                    $month = $model_liq->liq_mes + 1;
                    $year = $model_liq->liq_anio;
                }
                $arreglo_monto_sin_interes = A2Liquidaciones::Calcular_Monto_Periodo($model_liq->id_operacion, $model_liq->liq_anio, $model_liq->liq_mes, 0);
                $arreglo_monto_proximo = A2Liquidaciones::Calcular_Monto_Periodo($model_liq->id_operacion, $month, $year);
                
                if ($arreglo_monto_sin_interes['monto'] != $arreglo_monto_proximo['monto']) {
                    ?>
                    <tr>
                        <td colspan="3"><div align="center">Nuevo Precio: $<?php echo $arreglo_monto_proximo['monto']; ?></div></td>
                    </tr>
                    <?php
                }
                ?>
